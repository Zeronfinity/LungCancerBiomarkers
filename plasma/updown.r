...
lfc = vector('numeric')
abslfc = vector('numeric')

for (i in 1:length(names_up))
{
  names = c(names, names_up[i])
  lfc = c(lfc, lfc_up[i])
  abslfc = c(abslfc, abs(lfc_up[i]))
}
for (i in 1:length(names_down))
{
  names = c(names, names_down[i])
  lfc = c(lfc, lfc_down[i])
  abslfc = c(abslfc, abs(lfc_down[i]))
}
res_abs = data.frame(row.names=names, "log2 FC Value"=lfc, "abs log2 of FC"=abslfc)
...
#heatmap.2(data_, main="Significant Metabolites of Plasma Sample", trace="none", margins=c(4,14), density="none", 
